...
public class CustomMongoExportJournalistSocial {
	...
	public static void main(String[] args) throws IOException {
		...
		sentimentWriter.write("_id; date; author; isRetweet; type; text; sentimentscore; relevance; count; orientation; politician");
		sentimentWriter.newLine();
		noZeroSentimentWriter.write("_id; date; author; isRetweet; type; text; sentimentscore; relevance; count; orientation; politician");
		...
		ArrayList<String> journalists = new ArrayList<>();
		BufferedReader journalistReader = new BufferedReader(new FileReader("C:/Users/Local/Documents/Universität/KDD Seminar/news-creators/newspaper_authors_twitteraccs.txt"));
		String s;
		while((s = journalistReader.readLine()) != null) {
			journalists.add(s);
		}
		...
		for (int i = 1; i <= 50; i++) {
			...
			for(String journalist : journalists) {
				filterList.add(Filters.regex("creator", journalist, "i"));
			}
			
			// TODO: Achtung Einschränkung!
			FindIterable<Document> cursor = mCol.find(Filters.and(Filters.exists("sentiment"), Filters.or(filterList)));
			...
			for (Document x : cursor) {
				...
				String author = x.getString("creator");
				...
				addSentiments(sentimentWriter, _id, date, author, isRetweet, sentiment, false);
				addSentiments(noZeroSentimentWriter, _id, date, author, isRetweet, sentiment, true);
	...
	public static void addSentiments(BufferedWriter writer, String _id, Date date, String author, boolean isRetweet, ArrayList<Document> sentiments, boolean suppressZeroSentiments)
			throws IOException {
		boolean add;
